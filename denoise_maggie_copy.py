...
def optimalDenoise(noisy, k, alphabet, rho, alpha):
    for i in range(len(noisy)-1):
        if noisy[i] == noisy[i+1] and rho*alpha**2/(1-alpha) > 1 and alpha>1-alpha:
...
def denoiseSequence4(noisy, k, alphabet, deletion_rate):    
    pairprobs = {}
    for i in range(1,len(noisy)):
        paircontext = noisy[i-1:i+1]
        if paircontext in pairprobs:
            pairprobs[paircontext] += 1/(len(noisy)-1.0)
        else:
            pairprobs[paircontext] = 1/(len(noisy)-1.0)
    ...
    for i in range(len(noisy)-1):
        toadd = noisy[i]
        currpair = noisy[i:i+2]
        P1 = []
        prob_currpair = 0
        if currpair in pairprobs:
            prob_currpair+=pairprobs[currpair]
        alltripletprobs = 0
        for symbol in alphabet:
            suggestedtriplet = currpair[0]+symbol+currpair[1]
