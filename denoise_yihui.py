...
def denoiseSequence3(noisy,k, alphabet): #'noisy' must be a string
    for i in range(k, len(noisy)-k+1):
        if i < len(noisy)-k:
            context = noisy[i-k:i]+noisy[i+1:i+k+1]
            if context in context_hist:
            ...
            if context_del in context_hist:
                context_hist[context_del] += 'N'
            else:
                context_hist[context_del] = ['N']

    for context, symbolswithcontext in contexthist.iteritems():
        for a in alphabet:
            new_p = sum(context_hist[context] == a)/(len(context_hist[context])+0.0)
    ...
    noisy = list(noisy)
    noisy_mod = [[i,'N'] for i in noisy]
    noisy_mod = list(chain.from_iterable(noisy_mod))
    for i in range(2*k,len(noisy_mod)-2*k):
        if i%2 == 0: # Then it is an actual symbol
            # Consider if symbol i should be substituted

        else # Then it is a potential deletion location which we have marked with `N's
...
Y = ['a' if i==-1 else 'b' if i==1 else 'E' for i in Y]
alphabet = ['a','b']
Yest = denoiseSequence3(Y,k,alphabet)


################################################## Graph generation and plotting code ####################################################
# numpoints = 10
# errorrate = np.zeros(numpoints)

# ind=0
# for k in range(numpoints): #k is window size
#     print k
#     erasure_denoised = erasureDenoise(input, k, alphabet, rho) 
#     errorrate[ind] = 1-sum([int(input[i] == erasure_denoised[i]) for i in range(len(input))])/(len(input)+0.0)
#     ind+=1

# naiveerrorrate = rho*(len(alphabet)-1)/len(alphabet)*np.ones(numpoints)

# plt.plot(list(range(numpoints)),errorrate,label='Error rate')
# plt.plot(list(range(numpoints)),naiveerrorrate, label='Random guessing')
