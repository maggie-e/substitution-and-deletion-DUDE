...
def denoiseSequence3(noisy,k, alphabet, erasedsymbol, ppi, loss): #'noisy' must be a string
    ...
    for context, symbolswithcontext in context_hist.iteritems():
        ...
        probhist = np.zeros(len(alphabet)+1)
        values = count_symbols(symbollist, newalphabet)
    ...
    noisy = list(noisy)
    noisy_mod = [[i,erasedsymbol] for i in noisy[k:len(noisy-k)+1]]
    noisy_mod = itertools.chain(noisy[:k],[erasedsymbol],noisy_mod,noisy[len(noisy)-k+1:])
    noisy_mod = list(chain.from_iterable(noisy_mod))

    for i in range(k,len(noisy_mod)-k):
        # Consider if symbol i should be substituted
        context = noisy[j-k:j]+noisy[j+1:j+k+1]
...
Y = ['a' if i==-1 else 'b' if i==1 else erasedsymbol for i in Y]
...
alphabet = ['a','b']
Yest = denoiseSequence3(Y,k,alphabet,erasedsymbol, transitionmat,lossmat)
print Yest
